...
Now load the requried libraries for this practical:
...
* `frequency` - the number of observations per unit of time.  Some commen examples of values are:
...
**TODO** dump in the formula

Basically you take m data points:
k to the left and k to the right of x(t) and average them with x(t)
...
To make it easier to plot both of these, we can create a new data frame, where we add new columns to `Tvalues` that contain these two moving averages.  For this, we use the `dplyr` package along with the `mutate` function to add two new columns: `ma5` and `ma13`.
```{r}
...
**TODO** look at latest practical 7 slides, slide 12,13

Time series:

lag 1 correlation is a the correlation of the time series x(t) 

with the time series shifted by one time unit x(t + Dt)
...
**TODO** What do we see with `acf_T$acf[10]` ?  What's the interpretation?
...
**TODO** check against slide 18 in practical lecture
...
What we see is that the range of the observed values is much greater than that of the random and the trend values.  
...
**TODO** explain `na.omit` 

Some functions will generate an error if they have an NA in them.
...
**TODO** check that the example shows how to do all the things in the exercise.
...
**TODO** `dec2.1$time.series[,"trend"]` may not be clear - students may not know to look for this
...
**TODO** justification for `lag.max = 30`?  since we're interested in monthly patterns?
...
acf(c(NA, dec2.1$time.series[,"remainder"]), type = "correlation", lag.max = 30)
```
