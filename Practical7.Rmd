...
**TODO** ACF isn't covered yet, right?
...
To examine the relation between the `observed`, `random`, `seasonal` and `trend` data, we create a new data frame and plot histograms of each component.
```{r, warning=FALSE}
...
# Note that df1$value is a factor.  We want to have this represented as a number
class(df1$value)
...
**TODO** need to somewhere show `na.omit` 
