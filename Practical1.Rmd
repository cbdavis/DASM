...
This is a formula from an actual spreadsheet in use by a company.  This code is a nightmare to maintain and it's very difficult to check if there are any errors in it.  A reason to use R (or similar languages) is that spreadsheets create a soup of data storage, processing and presentation (i.e. numbers, formulas and graphs).  This hinders transparency and as spreadsheets get more complex, it becomes very difficult to trace how the data analysis is actually done.  With R, it's possible to use code to read from data files and then **[automatically generate reports in the form of Word documents, pdfs, or html pages](http://rmarkdown.rstudio.com/)**.

# First Steps:

## Setting up R and RStudio

You need to have both R and RStudio set up.  You can use R without RStudio, but if you do, the interface will look like the one on the left image below, and this can be a bit intimidating.  RStudio provides a more visual interface to R and it greatly assists you with various aspects of programming.
...
Everything shown in the large gray boxes below is code that you can run by copy/pasting into the `Console` tab in RStudio.  You can also collect these statements in a new R file (`File` -> `New File` -> `R Script`) which you can then run.
