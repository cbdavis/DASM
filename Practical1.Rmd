...
We first need to load the library that we'll be using for the rest of this tutorial.  Make sure to also include `options(stringsAsFactors = FALSE)`.  We'll cover factors in a later practical, but for now it's important to specify the `stringsAsFactors` option whenever running code as you may get confusing results without it.
...
* `fill=factor(cyl)` - The fill color for the bars is based on the value of `cyc`
* `geom_bar(width=1)` - This is needed to that there isn't a hole in the plot when we use the code in the step below.
* **Note:** the height of the bars is counting the number of cars with a specific value for `cyc`.  The size of the pie slices is not based on actual numeric values in `mtcars`.
