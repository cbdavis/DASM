...
# TODO still need to cover
* Some sort of exercise for the students (with dataset x, do y,z)
* calculate mean in row/column directions (rowMeans, rowSums, etc)
* `setwd`
...
You should realize that [Spreadsheets are Code](http://www.slideshare.net/Felienne/spreadsheets-are-code-online)
...
Spreadsheets create a soup of data storage, processing and presentation (i.e. numbers, formulas and graphs).  This hinders transparency and as spreadsheets get more complex, it becomes very difficult to trace how the data analysis is actually done.
...
Once R and RStudio is installed, **open up RStudio and install the necessary packages for R**.  Note that with R, the word "package" and "library" are often used interchangeably.  We'll need two packages:
...
Repeat the steps above to install the `dplyr` package as well via the GUI or just by typing in the console:
...
Everything shown in the gray boxes below is code that you can run by copy/pasting into the `Console` tab in RStudio.  You can also collect these statements in a new R file (`File` -> `New File` -> `R Script`) which you can then run.
...
## Data Frames
Can mix in different types of data per column
...
We'll look at the `mtcars` data set that is included with R.  If you type `?mtcars` in the console, you'll see more documentation.  Tabular data like you would find in spreadsheets is represented within R in what are known as **data frames**.  

Looking at the first few lines of the `mtcars` data frame, we see the following.  There are eleven columns: `mpg`, `cyl`, `disp`, `hp`, `drat`, `wt`, `qsec`, `vs`, `am`, `gear` and `carb`
```{r}
head(mtcars)
```
...
Depends on what is being plotted exactly - are you counting the number of something in a data frame, or is it already summarized?

**TODO** Pie chart doesn't check if numbers add up to 100% - just shows the overall percentage
...
# if you're plotting actual numbers, need stat="identity"
ggplot(mtcars %>% 
         group_by(cyl) %>% 
         summarize(carcount = n()), 
       aes(x=factor(1), y=carcount, fill=factor(cyl))) + 
...
[CO2 emissions data from the UNFCCC](http://unfccc.int/ghg_data/ghg_data_unfccc/time_series_annex_i/items/3814.php), specifically the ["CO2 excluding LULUCF" Excel spreadsheet](http://unfccc.int/files/ghg_emissions_data/application/x-zip-compressed/ai_co2_wolulucf.zip.zip) which we present here in a CSV file format.
...
```{r EmissionsPerCountryLinePlot, fig.width=12, fig.height=6}
ggplot(df, aes(x=Year, y=CO2_Mt, colour=Country)) + geom_line()
```

```{r EmissionsPerCountryArea, fig.width=12, fig.height=6}
ggplot(df, 
       aes(x=Year, y=CO2_Mt, fill=Country)) + 
  geom_area()
```

```{r EmissionsPerCountryAreaWithSeparatingLine, fig.width=12, fig.height=6}
ggplot(df, 
       aes(x=Year, y=CO2_Mt, fill=Country)) + 
  geom_area() + 
...
```{r EmissionsPerCountryFacetWrap, fig.width=12, fig.height=10}
...
```{r EmissionsPerCountryFacetWrapYmin0, fig.width=12, fig.height=10}
...
## Reference Sheets
**These reference sheets are important and summarize much of what you will need to know about R**
