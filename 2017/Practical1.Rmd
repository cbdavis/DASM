...
In the examples above, we just a variable simply named `a`.  R actually allows you to create much more descriptive names for variables

```{r}

```

valid variable names (no hyphen in variable names), can't start a variable name with a number

a name must start with ‘.’ or a letter, and if it starts with ‘.’ the second character must not be a digit. Names are effectively unlimited in length. 
...
show how to do basic calculations like means, sd, sum, diff, etc

cover nrow, ncol, dim, class, summary
