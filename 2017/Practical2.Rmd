...
Ocassionally you may find that you are working with data that is not of the correct data type.  You can fix this by using one of the following functions:
...
Note that you should always preceed these statements with your data frame (i.e. `mtcars %>% `).  Without this, `dplyr` won't know where to find the values for `hp`.
