...
* factor - typically used for "categories" of data

Within a single data frame, we can have different data types per column
```{r}
...
As shown in the previous practical, we can run `summary()` on this data frame to get an idea of what its contents are.  
...
The part about `Levels: 2 3 4 7` shows that its treating these numbers as labels or categories.  In other words, these values would be used to say things like Category 2 = "likes cheese", Category 3 = "doesn't like cheese", etc.
...
* `=` - only used for variable assignement
