...
* $H_0 = 20$ - our null hypothesis is that smokers also score 20 on the test.
...
x = seq(-3, 3, 0.1) # sequence from -3 to 3 in increments of 0.1
...
Try a two-sided test for a value of $mu=17$ (you should see the results below).  Also try with different values of $mu$.

```{r, echo=FALSE}
res = t.test(smoke, alternative = "two.sided", mu=17, conf.level = 0.95)
n = length(smoke)    # number of samples
...
If instead of typing the command `res = t.test(smoke, alternative = "two.sided", mu=17, conf.level = 0.95)`, you just typed `res = t.test(smoke)`, you would get the plot below, which doesn't seem to be correct.  
...
**TODO** - rework this?  Needs to be better tied into the text

Now we plot the t-distribution with alpha = 0.05 and degrees of freedom is 9 (since n-1).  For the upper tail t-test, your t value is negative since it's on the left side of the distribution.  The upper tail t-test calculates the probability of greater than this t-value:
...
Since our p-value is `r round(f_test_results$p.value, 3)`, we can't reject the null hypothesis that the ratio of the variances is equal to one.
