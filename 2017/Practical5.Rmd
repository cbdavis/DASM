...
**TODO** just show how to use chi-squared test directly, don't show how to do this by hand
still show how to do rowsums and column sums
...
**TODO** clean up/transition text

we learned how to do this stuff step-by-step in the lecture, here we learn how to just do it in R
...
This is the example on number of calories consumed by month (page 37 of Lecture 6).  **check lecture reference**
...
Using what you learned above, first put the data into a format that can be used with the one way test, and conduct the test.  Create a normal probability plot (`qqnorm` and `qqline`) to see if the data are roughly normally distributed.
...
ggplot(infants, aes(x=group, y=walking_age_months)) + geom_boxplot()
#ggplot(infants, aes(sample=walking_age_months)) + stat_qq() + facet_wrap(~group)

```

#### group = active
```{r}
qqnorm(infants %>% filter(group=="active") %>% select(walking_age_months) %>% unlist())
qqline(infants %>% filter(group=="active") %>% select(walking_age_months) %>% unlist())
```

#### group = passive
```{r}
qqnorm(infants %>% filter(group=="passive") %>% select(walking_age_months) %>% unlist())
qqline(infants %>% filter(group=="passive") %>% select(walking_age_months) %>% unlist())
```

#### group = none
```{r}
qqnorm(infants %>% filter(group=="none") %>% select(walking_age_months) %>% unlist())
qqline(infants %>% filter(group=="none") %>% select(walking_age_months) %>% unlist())
```

#### group = ctr.8w
```{r}
qqnorm(infants %>% filter(group=="ctr.8w") %>% select(walking_age_months) %>% unlist())
qqline(infants %>% filter(group=="ctr.8w") %>% select(walking_age_months) %>% unlist())
```
