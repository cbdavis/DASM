...
For this example, download the [lynx.csv](https://raw.githubusercontent.com/cbdavis/DASM/master/2017/data/lynx.csv) data set and load it into R.  Once you read it in, you'll have a data frame with a single column `x` which is specified in the csv file itself.
```{r, echo=FALSE}
Tvalues = read.csv("./data/lynx.csv")
```
```{r, eval=FALSE}
Tvalues = read.csv("lynx.csv")
```

As we showed in the previous practical, we can create a sequence of dates (using `seq`), starting at one date with a defined time interval for each step.  This data set describes the number of lynx caught each year on the MacKenzie River from 1821 to 1934.

```{r}
Tvalues$t = seq(from = as.Date("1821-01-01"), to = as.Date("1934-01-01"), by = "1 year")
```
...
Here we use data from [co2.csv](https://github.com/cbdavis/DASM/raw/master/2017/data/co2.csv) which contains CO$_2$ measurements from the Mauna Loa observatory from January 1959 until December 1997, with values for each month.

Using `freq = 12` means that you have 12 observations per cycle (i.e. per year), so the period of the seasonal variation is just 12.

```{r}
co2_vals = read.csv("https://github.com/cbdavis/DASM/raw/master/2017/data/co2.csv")

t_ser <- ts(co2_vals$co2_ppm, start = c(1959, 1), freq = 12)
plot(t_ser)
```

You can also define a time series without a start date.  Now the values on the x axis correspond to the cycle number, which in this case refers to the number of years after the first observation.  Looking at the x axis, we see that there are nearly 40 years of observations.

```{r}
t_ser1 <- ts(co2_vals$co2_ppm, freq = 12)
plot(t_ser1)
```
...
Here we create a single moving average on the `lynx.csv` data we loaded in above.
...
ggplot(Tvalues_with_ma, aes(x=t)) + 
  ...
  xlim(c(as.Date("1821-01-01"), 
         as.Date("1860-01-01"))) # Just show data from 1821-1860, otherwise the plot is too cluttered
...
Here we shift the values for `co2_ppm` in `co2_vals` by three months.  We use the `mutate()` function to create a new column named `lag_3` in the data frame.  
```{r}
co2_vals_lag3 = co2_vals %>% mutate(lag_3 = lag(co2_ppm, 3))
```
...
ggplot(data = co2_vals %>% mutate(lag_x = lag(co2_ppm, 3)), 
       aes(x=co2_ppm, y=lag_x)) + 
  geom_point()

## data shifted 6 months
ggplot(data = co2_vals %>% mutate(lag_x = lag(co2_ppm, 6)), 
       aes(x=co2_ppm, y=lag_x)) + 
  geom_point()

## data shifted 10 months
ggplot(data = co2_vals %>% mutate(lag_x = lag(co2_ppm, 10)), 
       aes(x=co2_ppm, y=lag_x)) + 
  geom_point()

## data shifted 12 months
ggplot(data = co2_vals %>% mutate(lag_x = lag(co2_ppm, 12)), 
       aes(x=co2_ppm, y=lag_x)) + 
  geom_point()
```
...
Using the `lynx.csv` data, we estimate the autocorrelation with a maximum lag of 64.
...
Here we use the `co2_ppm` data.

```{r}
## create a time series object with frequency of 1, since we're dealing with yearly data
t_ser1 <- ts(co2_vals$co2_ppm, freq = 12)
...
acf_des <- acf(as.numeric(output$random), type = "correlation", lag.max = 128, na.action=na.omit)
```
...
```{r}
acf_des <- acf(output$random, type = "correlation", lag.max = 128, na.action=na.omit)
```

What we see above is that there is still some seasonality in the calculated random noise component, but not very much.
...
We again use the `co2_vals` data.
