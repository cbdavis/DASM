...
From this we can see that there's a weekly pattern and a seasonal pattern where we get peaks in the spring and fall.
...
**TODO** talk about using different values for freq, since we have multiple periods, give some examples
...
Here we shift the values for `volume` in `sustainability` by seven days.  We use the `mutate()` function to create a new column named `lag_3` in the data frame.  
```{r}
