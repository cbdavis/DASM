...
co2_vals = read.csv("https://raw.githubusercontent.com/cbdavis/DASM/master/2017/data/co2.csv")

t_ser <- ts(Tvalues$x, start = c(1821, 1), freq = 1)
plot(t_ser)
```

You can also define a time series without a start date.  Using `freqquency = 1` means that you have 1 observations per cycle (i.e. per year), so the period of the seasonal variation is just 1.  Now the values on the x axis correspond to the cycle number, which in this case refers to the number of years after the first observation. 

```{r}
t_ser1 <- ts(Tvalues$x, freq = 1)
plot(t_ser1)
```
...
**TODO** use lynx data here
**TODO** search replace Pr_20May1.csv with lynx data
...
Here we use the `lynx.csv` data which we've loaded into `Tvalues` above.

### Classical

#### Example

**TODO** use lynx data, also for stl example
...
**TODO** use the lynx data here as well

We again use the `Tvalues` data which is from `lynx.csv`.  
