...
In R, we represent $P(B|A)$ as the variable `p_B_given_A` and we make a sequence from 0.8 to 1 in steps of 0.005 (`seq(0.8,1,0.005)`), which indicates a range of values for the test being 80% accurrate all the way up to 100% accurate at spotting BSE given that the cow actually has BSE.
...
If you type `?pnorm` in the console, you'll find that the documention for `lower.tail` states that "*if TRUE (default), probabilities are P[X ≤ x] otherwise, P[X > x]*".  This applies for the other distributions as well.
...
The numbers above correspond to the rows in the `faithful` dataset where the eruptions are less than three minutes.
...
If you type `help(package="datasets")` into the RStudio Console you'll see a list of other datasets which are included with R.  You can experiment with these just as we did with the `faithful` dataset to see if this data is normally distributed as well.
...
```
This isn't a good approach if we have to sum up the probabilities for a lot of numbers like 7, 8, 9, ... 1000.  
...
```{r}
# pass a vector of 7, 8, 9, 10 and then sum up the probabilities
sum(dbinom(c(7:10), size=10, prob=0.5))
```
...
Probability of observing exactly 43 cars:
```{r}
dbinom(43, size=n, prob=0.5)
```

Probability of observing at most 43 cars:
```{r}
...
* What is the distribution you would use to estimate the probability of counting x neutrions in a given day?
