...
positive_has_BSE = df$p_A_given_B

# Cow tests negative and has BSE
negative_has_BSE = 1 - df$p_A_given_B

# Cow tests positive and doesn't have BSE
positive_doesnt_have_BSE = 1 - df$p_B_given_A

# Cow tests negative and doesn't BSE
negative_doesnt_have_BSE = df$p_B_given_A
```
...
```{r}
results = rbind(cbind(p_B_given_A, positive_has_BSE, "Test +, has BSE"),
                cbind(p_B_given_A, negative_has_BSE, "Test -, has BSE"),
                cbind(p_B_given_A, positive_doesnt_have_BSE, "Test +, no BSE"),
                cbind(p_B_given_A, negative_doesnt_have_BSE, "Test -, no BSE"))

# convert this into a data frame
results = as.data.frame(results)

# add columns to the data frame
colnames(results) = c("test_accuracy", "probability", "probability_type")
```

You'll notice that there's one thing we need to fix here:

```{r}
head(results$test_accuracy)
```
...
```{r}
results$test_accuracy = as.numeric(results$test_accuracy)
results$probability = as.numeric(results$probability)
```

Now we can plot things:
```{r}
ggplot(results, aes(x=test_accuracy, y=probability * numCows, fill=probability_type)) + geom_area(position="stack")
```
