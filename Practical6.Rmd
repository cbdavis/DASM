...
One thing which we'll come back to later is that the linear regressions for the four data sets in Anscombe's Quartet are nearly identifical, even though visually they look quite different:
```{r, echo=FALSE}
...
Remember from the lecture that the cutoff value can be found using `qf(0.5, p, n-p, lower.tail = FALSE)` where:
* `p` - fitted parameters (=2 with simple linear reg)
